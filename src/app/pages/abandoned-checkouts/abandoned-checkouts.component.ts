...
@Component({
  selector: 'app-abandoned-checkouts',
  standalone: true,
  templateUrl: './abandoned-checkouts.component.html',
  styleUrls: ['./abandoned-checkouts.component.css'],
  imports: [CommonModule, FormsModule, RouterModule, TranslateModule]
})
export class AbandonedCheckoutsComponent implements OnInit {

  abandonedCheckouts: AbandonedCheckout[] = [];
  isLoading = false;

  isModalVisible = false;
  editingAbandonedCheckout: AbandonedCheckout | null = null;
  ...
  workflowStatus: { inProgress: boolean; message: string } = {
    inProgress: false,
    message: ''
  };
  ...
  async loadAbandonedCheckouts(): Promise<void> {
    this.isLoading = true;
    try {
      this.abandonedCheckouts = await this.abandonedCheckoutService.getAbandonedCheckouts();
    } catch (error) {
  ...
  async showModal(record: AbandonedCheckout | null = null): Promise<void> {
    this.showValidationErrors = false;

    if (record) {
      this.editingAbandonedCheckout = record;
      const detail = await this.abandonedCheckoutService.getAbandonedCheckoutDetails(record.id);
  ...
  hideModal(): void {
    ...
    this.editingAbandonedCheckout = null;
  }
  ...
  async save(): Promise<void> {
    ...
    // El client secret solo se manda si el usuario escribio uno nuevo — al editar, si se
    // deja vacio, no se sobreescribe el que ya estaba guardado.
    if (this.shopifyClientSecret) {
      payload.shopifyClientSecret = this.shopifyClientSecret;
    }

    if (this.editingAbandonedCheckout) {
      payload._id = this.editingAbandonedCheckout.id;
    }

    try {
      const response = await this.abandonedCheckoutService.saveAbandonedCheckout(payload);
      const isNew = !this.editingAbandonedCheckout;
      ...
      if (isNew && abandonedCheckoutId) {
        this.triggerWorkflow(abandonedCheckoutId);
      }
    } catch (error: any) {
  ...
  private triggerWorkflow(abandonedCheckoutId: number): void {
    ...
    this.abandonedCheckoutService.createAbandonedCheckoutOnWebhook(abandonedCheckoutId).subscribe({
      next: (webhookResponse: any) => {
        // Exito 200: { "success": "the abandoned checkout followup was successfully updated" }
        this.notificationService.showSuccess(
      ...
      error: (error) => {
        // Error 404: { "error": "mensaje..." } — el mensaje va directo en error.error.error,
        // no en error.error.message.
  ...
  editAbandonedCheckout(record: AbandonedCheckout, event: Event): void {
    event.stopPropagation();
    this.showModal(record);
  }

  viewDetails(record: AbandonedCheckout): void {
    this.router.navigate(['/abandoned-checkouts', record.id]);
  }

  getStatusBadgeClass(status: AbandonedCheckoutStatus | undefined | null): string {
    const baseClass = 'ac-status-badge';
    let statusClass = 'ac-status-default';

    switch (status) {
      case 'PLANNED':
        statusClass = 'ac-status-planned';
        break;
      case 'IN_PROGRESS':
        statusClass = 'ac-status-progress';
        break;
      case 'STOPED':
        statusClass = 'ac-status-stopped';
        break;
    }
