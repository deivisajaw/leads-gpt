...
import { CompaniesService, CompanySearchHistoryItem, CompanySearchHistoryResponse } from '../../services/companies.service';
import { Subscription } from 'rxjs';
import { filter } from 'rxjs/operators';

@Component({
  selector: 'app-my-search-history-companies',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, TranslateModule],
  templateUrl: './my-search-history-companies.component.html',
  styleUrl: './my-search-history-companies.component.css'
})
export class MySearchHistoryCompaniesComponent implements OnInit, OnDestroy {
  historyItems: CompanySearchHistoryItem[] = [];
  filteredHistoryItems: CompanySearchHistoryItem[] = [];
  searchQuery = '';
  isLoading = false;
  currentPage = 1;
  itemsPerPage = 25;
  ...
  async loadHistory(page: number = this.currentPage, sortBy: string = this.currentSortOrder) {
    ...
    try {
      const result: CompanySearchHistoryResponse = await this.companiesService.getMySearchHistoryCompanies(
        offset,
        this.itemsPerPage,
      ...
      if (result.error) {
        console.error('Error loading company search history:', result.message);
        this.historyItems = [];
      ...
      console.error('Error loading company search history:', error);
      this.historyItems = [];
      this.filteredHistoryItems = [];
      this.totalResultsInServer = 0;
    } finally {
  ...
  viewDetails(searchId: number) {
    this.router.navigate(['/my-history-search-company-details', searchId]);
  }
